...
function UpcomingScheduleWidget({ items }: UpcomingScheduleWidgetProps) {
    ...
    <div className="flex h-full flex-col gap-3">
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-2">
          {filtered.map((item) => (
            <div key={item.id} className={`rounded border px-2 py-2 ${item.overdue ? 'border-rose-400/40 bg-rose-500/10' : 'border-white/15 bg-white/[0.03]'}`}>
