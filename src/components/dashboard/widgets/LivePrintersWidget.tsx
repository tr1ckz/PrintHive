    ...
    <div className="flex h-full flex-col gap-2">
        ...
        <div className="flex flex-wrap gap-1 rounded border border-white/15 bg-black/20 p-1">
          {(['all', 'online', 'active'] as const).map((item) => (
            <button
              ...
              className={`rounded px-2 py-1 text-[10px] font-semibold uppercase tracking-[0.08em] ${scope === item ? 'bg-white/15 text-white' : 'text-white/60'}`}
            >
              {item}
            </button>
          ))}
      ...
      <div className="space-y-2">
        {visibleRows.map((printer) => (
          <div key={printer.id} className="rounded border border-white/15 bg-white/[0.03] p-2">
